...
public class AstraService {
	 ...
	 * 
	 * @param spacecraftName unique spacecraft name (PK)
	 ...
	 *
	 * @param spacecraftName unique spacecraft name (PK)
	 * @param journeyid      journey unique identifier
	 * @return journey details if it exists
	 */
	 ...
	 *
	 * @param spacecraftName name of spacecrafr
	 * @param journeyId      journey identifier
	 * @param pageSize       page size
	 * @param pageState      page state
	 * @return result page
	 */
	public PagedResultWrapper<SpacecraftTemperatureOverTime> getTemperatureReading(String spacecraftName,
			UUID journeyId, Optional<Integer> pageSize, Optional<String> pageState) {
		PagingIterable<SpacecraftTemperatureOverTime> daoResult = getSpaceCraftInstrumentsDao()
				.getTemperatureReading(spacecraftName, journeyId, pageSize, pageState);
		return new PagedResultWrapper<SpacecraftTemperatureOverTime>(daoResult,
				pageSize.isPresent() ? pageSize.get() : 0);
	}

	/**
	 * Create a new {@link SpacecraftTemperatureOverTime} for each item in the
	 * array.
	 *
	 * @param readings An array unique temperature readings
	...
	public ActivityLog getActivity(UUID userId, String activityYear, UUID activityId) {
		PagingIterable<ActivityLog> daoResult = getActivityLogDao().getActivityLog(userId, activityYear, activityId);

		return daoResult.one();
	}

	/**
	 * Create a new {@link SpacecraftLocationOverTime} for each item in the array.
	 *
	 * @param readings An array unique location readings
	 * @return true if successful
	 */
	public void insertLocationReading(SpacecraftLocationOverTime[] readings) {
		long top = System.currentTimeMillis();
		getSpaceCraftInstrumentsDao().insertLocationReadingAsync(readings)
				.whenComplete((res, ex) -> LOGGER.debug("{} location reading(s) inserted in {} millis", readings.length,
	 ...
	 *
	 * @param readings An array unique pressure readings
	 * @return true if successful
	 */
	public void insertPressureReading(SpacecraftPressureOverTime[] readings) {
		long top = System.currentTimeMillis();
		getSpaceCraftInstrumentsDao().insertPressureReadingAsync(readings)
				.whenComplete((res, ex) -> LOGGER.debug("{} pressure reading(s) inserted in {} millis", readings.length,
						System.currentTimeMillis() - top));
	}
	...
	public void insertSpeedReading(SpacecraftSpeedOverTime[] readings) {
		long top = System.currentTimeMillis();
		getSpaceCraftInstrumentsDao().insertSpeedReadingAsync(readings).whenComplete((res, ex) -> LOGGER
				.debug("{} speed reading(s) inserted in {} millis", readings.length, System.currentTimeMillis() - top));
	}
	 ...
	 * @return result page
	 */
	public PagedResultWrapper<SpacecraftPressureOverTime> getPressureReading(String spacecraftName, UUID journeyId,
			Optional<Integer> pageSize, Optional<String> pageState) {
		PagingIterable<SpacecraftPressureOverTime> daoResult = getSpaceCraftInstrumentsDao()
				.getPressureReading(spacecraftName, journeyId, pageSize, pageState);
		return new PagedResultWrapper<SpacecraftPressureOverTime>(daoResult, pageSize.isPresent() ? pageSize.get() : 0);
	}
	 ...
	 * @return result page
	 */
	public PagedResultWrapper<SpacecraftSpeedOverTime> getSpeedReading(String spacecraftName, UUID journeyId,
			Optional<Integer> pageSize, Optional<String> pageState) {
		PagingIterable<SpacecraftSpeedOverTime> daoResult = getSpaceCraftInstrumentsDao()
				.getSpeedReading(spacecraftName, journeyId, pageSize, pageState);
		return new PagedResultWrapper<SpacecraftSpeedOverTime>(daoResult, pageSize.isPresent() ? pageSize.get() : 0);
	}
	 ...
	 * @return result page
	 */
	public PagedResultWrapper<SpacecraftLocationOverTime> getLocationReading(String spacecraftName, UUID journeyId,
			Optional<Integer> pageSize, Optional<String> pageState) {
		PagingIterable<SpacecraftLocationOverTime> daoResult = getSpaceCraftInstrumentsDao()
				.getLocationReading(spacecraftName, journeyId, pageSize, pageState);
		return new PagedResultWrapper<SpacecraftLocationOverTime>(daoResult, pageSize.isPresent() ? pageSize.get() : 0);
	}
